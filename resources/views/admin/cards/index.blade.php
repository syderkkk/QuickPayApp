                            ...
                            <tbody>
                                @forelse($cards as $card)
                                    <tr class="hover:bg-[#f5f7fa] transition">
                                        ...
                                        <td class="px-3 py-2 align-middle">{{ $card->user->name ?? '-' }} {{ $card->user->lastname ?? '-' }}</td>
                        ...
                        <div class="w-full flex justify-center mt-6">
                            <nav class="flex gap-1 font-mono" aria-label="Paginación">
                                ...
                                @if ($cards->onFirstPage())
                                    <span
                                        class="px-3 py-2 rounded-full bg-gray-200 text-gray-400 cursor-not-allowed select-none">Anterior</span>
                                @else
                                ...
                                @foreach ($cards->getUrlRange(1, $cards->lastPage()) as $page => $url)
                                    @if ($page == $cards->currentPage())
                                        <span
                                            class="px-3 py-2 rounded-full bg-[#ede8f6] text-[#2563eb] font-bold">{{ $page }}</span>
                                    @else
                                        <a href="{{ $url }}"
                                ...
                                @else
                                    <span
                                        class="px-3 py-2 rounded-full bg-gray-200 text-gray-400 cursor-not-allowed select-none">Siguiente</span>
                                @endif
                            </nav>
                        </div>
                    @endif
