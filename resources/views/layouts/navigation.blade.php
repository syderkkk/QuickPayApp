...
<nav x-data="{ open: false }" class="bg-primary w-full font-secondary shadow-lg relative z-50">
                ...
                <div x-show="openNotif" @click.away="openNotif = false"
                    ...
                    <div class="bg-gradient-to-r from-primary to-blue-600 px-6 py-4">
                        <div class="flex items-center justify-between">
                            ...
                            @if ($unreadCount > 0)
                                <span class="bg-white text-primary px-3 py-1 rounded-full text-xs font-bold">
                                    {{ $unreadCount }} nuevas
                                </span>
                            @endif
                        </div>
                        ...
                        <div class="space-y-3" id="notifications-list">
                            @forelse($notifications as $notification)
                                @if ($notification->type === 'request')
                                    <a href="{{ url('/transactions/request/' . ($notification->data['request_id'] ?? '')) }}"
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-200"
        ...
        x-transition:leave-end="opacity-0 -translate-y-2" class="md:hidden bg-primary border-t border-white/10">
                ...
                <div x-data="{ mobileNotifOpen: false }" class="relative">
                    ...
                    <div x-show="mobileNotifOpen" 
                        ...
                        <div @click.away="mobileNotifOpen = false" 
                            ...
                            <div class="max-h-[calc(80vh-120px)] overflow-y-auto p-3">
                                <div class="space-y-3">
                                    @forelse($notifications as $notification)
...
<script>
    ...
    function handleNotificationClick(notificationId, requestId) {
        if (requestId) {
            ...
            window.location.href = '{{ url('/transactions/request') }}/' + requestId;
        }
    }

    function handleMobileNotificationClick(notificationId, requestId) {
        if (requestId) {
            // Mark as read and redirect
            markAsRead(notificationId);
            window.location.href = '{{ url('/transactions/request') }}/' + requestId;
