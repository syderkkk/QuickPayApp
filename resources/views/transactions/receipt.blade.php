    ...
    <div class="container">
        ...
        <div class="header">
            ...
            <div class="ruc-box">
                ...
                <div
                    class="badge {{ $transaction->status === 'completed' ? '' : ($transaction->status === 'pending' ? 'pending' : 'cancelled') }}">
        ...
        <table class="info-table">
            ...
            <tr>
                ...
                <td class="value">
                    {{ $transaction->custom_id ?? strtoupper(substr(md5($transaction->id . $transaction->created_at), 0, 12)) }}
                </td>
            ...
            <tr>
                ...
                <td class="value">
                    <strong>{{ strtoupper($transaction->status === 'completed' ? 'COMPLETADO' : ($transaction->status === 'pending' ? 'PENDIENTE' : $transaction->status)) }}</strong>
                </td>
            </tr>
            <tr>
                    ...
                    <td>
                        @if ($transaction->type === 'request')
                            ...
                            @else
                                ...
                                <span
                                    class="amount-negative">-{{ number_format($transaction->converted_amount ?? $transaction->amount, 2) }}</span>
                            @endif
                        @else
                            {{-- Envío directo --}}
                            ...
                            @else
                                <span
                                    class="amount-positive">+{{ number_format($transaction->converted_amount ?? $transaction->amount, 2) }}</span>
                            @endif
                        @endif
                    </td>
                    <td class="payment-method">
                        @if ($transaction->card)
                            <div class="bank-info">{{ strtoupper($transaction->card->brand) }} - Tarjeta
                                {{ $transaction->card->type ?? 'Débito' }} **** {{ $transaction->card->last_four }}
                            </div>
                            <div class="bank-info">{{ $transaction->card->bank_name ?? 'Banco asociado' }}</div>
                            <div class="disclaimer">* Aparecerá como "QUICKPAY
                                {{ strtoupper($transaction->type === 'request' && $transaction->receiver_id !== auth()->id() ? $transaction->receiver->name : $transaction->sender->name ?? 'USUARIO') }}"
                                en su estado de cuenta</div>
                        @else
        ...
        @if (
            $transaction->type === 'request' &&
                ...
                $transaction->currency !== auth()->user()->wallet->currency &&
            ...
            <table class="details-table">
                ...
                <tr>
                    ...
                    <td><strong>1 {{ auth()->user()->wallet->currency }} =
                            {{ number_format(1 / $transaction->exchange_rate, 2) }}
                            {{ $transaction->currency }}</strong></td>
                    <td><span class="currency-symbol">{{ $transaction->currency }}</span>
                        <strong>{{ number_format($transaction->amount, 2) }}</strong></td>
                </tr>
                <tr>
                    <td><strong>Monto debitado de mi cuenta</strong></td>
                    <td><strong>1 {{ $transaction->currency }} = {{ number_format($transaction->exchange_rate, 2) }}
                            {{ auth()->user()->wallet->currency }}</strong></td>
                    <td><span class="currency-symbol amount-negative">{{ auth()->user()->wallet->currency }}</span>
                        <strong>{{ number_format($transaction->converted_amount, 2) }}</strong></td>
                </tr>
            </table>
        @elseif(
            $transaction->type !== 'request' &&
                isset($transaction->converted_amount) &&
                isset($transaction->exchange_rate) &&
                $transaction->currency !== $transaction->receiver_currency)
            ...
            <table class="details-table">
                ...
                <tr>
                    <td><strong>1 {{ $transaction->currency }} = {{ number_format($transaction->exchange_rate, 2) }}
                            {{ $transaction->receiver_currency }}</strong></td>
                    <td><span class="currency-symbol">{{ $transaction->receiver_currency }}</span>
                        <strong>{{ number_format($transaction->converted_amount, 2) }}</strong></td>
                </tr>
            </table>
        @endif
            ...
            <tr>
                ...
                <td class="value">
                    @if ($transaction->type === 'request')
                        ...
                        @else
                            <span class="currency-symbol">{{ auth()->user()->wallet->currency }}</span>
                            {{ number_format($transaction->converted_amount ?? $transaction->amount, 2) }}
                        @endif
                    @else
                        @if ($transaction->sender_id === auth()->id())
                            <span class="currency-symbol">{{ $transaction->currency }}</span>
                        ...
                        @else
                            <span
                                class="currency-symbol">{{ $transaction->receiver_currency ?? $transaction->currency }}</span>
                            {{ number_format($transaction->converted_amount ?? $transaction->amount, 2) }}
                        @endif
                    @endif
                </td>
